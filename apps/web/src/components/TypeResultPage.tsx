...
import { TypeInfo } from "@/features/analysis/useType";
...
interface TypeResultPageProps {
  result: TypeInfo;
}

export const TypeResultPage = ({ result }: TypeResultPageProps) => {
  return (
    <>
      <h1 className={cn("text-xl")}>당신은...</h1>
      <div className="flex flex-col gap-3 my-12 w-full">
        <CardItem
          title={`${result.title} 유형이에요. [${result.code}]`}
        ...
        />
        <CardItem title="대표 문장" description={result.language_of_meaning} />
        <CardItem
          title="상징적인 순간"
        ...
        />
        <CardItem title="의미의 근간" description={result.meaning_origin} />
        <CardItem title="핵심 감정" description={result.core_emotion} />
        <CardItem
          title="가장 어려움을 느끼는 상황"
          description={result.hardest_to_endure}
        />
        <CardItem title="위기 상황" description={result.crisis_state} />
        <CardItem title="숨기고 싶은 두려움" description={result.hidden_fear} />
        <CardItem
